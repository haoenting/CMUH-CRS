...
error_reporting(E_ERROR | E_PARSE);
  ...
  $Time_of_last_follow = $_POST['Time_of_last_follow-up'];
  $Date_of_mortality = $_POST['Date_of_mortality'];
  $Survival_time = $_POST['Survival_time'];
  $DT_summary = $_POST['Dx/Tx_summary'];
  $Permanent_adress = $_POST['Permanent_adress'];
  $TEL = $_POST['TEL'];
  $Time = date('Y/m/d H:i:s');

  $sql1 = "INSERT INTO `basic_information`
        VALUES('$Chart', '$ID','$Name',
        '$Birthday','$gender','$Native',
        '$Dx','$Visiting_staff','$Operator',
        '$Source', '$HNPCC','$FAP',
        '$Date_of_initial_diagnosis','$Metastases',
        '$Date_of_metastases_noted','$Time_of_last_follow',
        '$Date_of_mortality','$Survival_time',
        '$DT_summary','$Permanent_adress','$TEL','$Time')";
...
//pass
  if(!empty($_POST['Medical_history'])){
    $Medical_history = implode(", ", $_POST['Medical_history']);
  }
  else{
  ...
  if(!empty($_POST['Family_CRC'])){
    $Family_CRC =  implode(", ",$_POST['Family_CRC']);
  }
  else{
    $Family_CRC =  NULL ;
  }
  $Family_other_GI = $_POST['Family_other_cancer_GI'];
  $Family_other_cancer = $_POST['Family_other_cancer'];
 
  $sql2 = "INSERT INTO `pass`
    VALUES('$Medical_history', '$Surgical_history', '$Smoking', '$Drinking',
    '$Betal_nut', '$Family_CRC',
    '$Family_other_GI', '$Family_other_cancer', '$Chart')";
  ...
  $Musus_passage = $_POST['Musus_passage'];
  ...
  $CXR = $_POST['CXR'];
  if(!empty($_POST['Abdominal_CT'])){
    $Abdominal_CT = implode(", ", $_POST['Abdominal_CT']);
  }
  else{
    $Abdominal_CT = NULL ;
  }
  ...
  $sql4 = "INSERT INTO `preoperative assessment`
  VALUES('$Hemoglobin', '$WBC', '$Platelet', '$GOT', '$GPT',
  ...
  '$Distence_from_AV', '$Polyps1', '$CXR', '$Abdominal_CT',
  '$Other4_2', '$Barium_enema', '$Polyps2', '$Abdominal_sonography',
  '$Noudle_number', '$Other4_3', '$Other4_4', '$Chart')";
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料4已更新<br/>";
      ...
      echo "{$sql4} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }

  //preoperative treatment
  if(!empty($_POST['Nutrition_support'])){
    $Nutrition_support = implode(", ", $_POST['Nutrition_support']);
  }
  else{
  ...
  $sql5 = "INSERT INTO `preoperative treatment`
  VALUES('$Nutrition_support', '$P_RBC',
  '$Whole_blood', '$FFP', '$Platelet','$Type', '$Laxativs',
  '$Oral_antibiotics', '$Retention_enema', '$Pre_OP',
  '$Pre_OP_Other','$Chart')";
  ...
  if (mysqli_affected_rows($link)>0){
    echo "資料5已更新<br/>";
      ...
      echo "{$sql5} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }

  //ccrt
  if(!empty($_POST['Neoadjavent'])){
    $Neoadjavent = implode(", ", $_POST['Neoadjavent']);
  }
  else{
    $Neoadjavent = NULL;
  }
  ...
  $sql6 = "INSERT INTO `ccrt`
  VALUES('$Neoadjavent', '$Neoadjavent_dose',  
  '$Side_effect', '$Time_to_surgery', '$Chart')";
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料6已更新<br/>";
      ...
      echo "{$sql6} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $Type_anastomosis = $_POST['Type_anastomosis'];
  ...
  $sql7 = "INSERT INTO `surgery`
  VALUES('$Operatin_date', '$OP_method1', '$OP_method2','$Laparoscopy',
  ...
  '$Tumor_site', '$Type_anastomosis', '$Anastomosis_to_anal_verge',
  ...
  '$Intracolonic', '$Reason_of_palliative_TX', '$Chart')";
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料7已更新<br/>";
      ...
      echo "{$sql7} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }

  //postoperative condition
  $Cefazoline = $_POST['Cefazoline'];
  ...
  $sql8 = "INSERT INTO `postoperative condition`
  VALUES('$Cefazoline','$Gentamicin','$Metronidazole',
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料8已更新<br/>";
      ...
      echo "{$sql8} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql9 = "INSERT INTO `pathology`
  VALUES('$Tumor_long2',  '$Tumor_wide2','$Tumor_high2',
  '$Tumor_Type','$Distal_Margin','$Call_type',
  '$Differetiatnion','$Lypmhatic','$Vascular',
  '$Perineural','$Synchronous_Polypo','$Synchronous_cancer',
  '$Cut_margin_condition','$LN_examed','$LN_involved',
  '$Final_T','$Final_N','$Final_M','$Final_stage','$Chart')";
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料9已更新<br/>";
      ...
      echo "{$sql9} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql10 = "INSERT INTO `Chemotherapy`
  VALUES('$Chart', '$start1', '$start2', '$start3', '$$start4',
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料10已更新<br/>";
      ...
      echo "{$sql10} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql11 = "INSERT INTO `Adjavent radiotherapy`
  VALUES('$Chart', '$s1', '$e1', '$d1','$s2', '$e2', '$d2',
  '$s3', '$e3', '$d3', '$s4', '$e4', '$d4',
  '$s5', '$e5', '$d5','$s6', '$e6', '$d6',
  '$s7', '$e7', '$d7', '$s8', '$e8', '$d8',
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料11已更新<br/>";
      ...
      echo "{$sql11} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql12 = "INSERT INTO `tumor marker`
  VALUES('$Chart', '$Td1', '$TC1', '$T1251', '$T1991', '$To1',
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料12已更新<br/>";
      ...
      echo "{$sql12} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql13 = "INSERT INTO `Abdominal CT`
  VALUES('$Chart', '$ACt1', '$ACF1', '$ACPn1', '$ACPs1', '$ACo1',
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料13已更新<br/>";
      ...
      echo "{$sql13} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  ...
  $sql14 = "INSERT INTO `PET`
  VALUES('$Chart', '$PETt1', '$PETf1', '$PETo1',
  '$PETt2', '$PETf2', '$PETo2', 
  '$PETt3', '$PETf3', '$PETo3', 
  '$PETt4', '$PETf4', '$PETo4', 
  '$PETt5', '$PETf5', '$PETo5', 
  ...
  if (mysqli_affected_rows($link)>0) {
    echo "資料14已更新<br/>";
      ...
      echo "{$sql14} 語法執行失敗，錯誤訊息: " . mysqli_error($link);
  }
  mysqli_close($link);
